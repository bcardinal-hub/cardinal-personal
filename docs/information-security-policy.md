...
- Rate limiting on authentication endpoints (login, signup, password reset, sign-in code entry and resend), counted separately per endpoint per IP, to limit credential-stuffing and brute-force attempts.
- **Email one-time sign-in codes (consumer MFA) — built, not yet switched on.** When enabled (`REQUIRE_LOGIN_CODE=true`), a correct password no longer creates a session: a 6-digit code is emailed, and only entering it signs the user in — so no app route, including Plaid Link, is reachable on a password alone. Codes are single-use, expire in 10 minutes, lock after 5 wrong tries, are stored only as a keyed hash, and a new session ID is issued at sign-in. Signup requires the same code (proving the email is the user's), and a password reset does not sign the user in, so the reset link can't bypass the code. **It is off today** because the transactional email sender is still Resend's shared test address, which only delivers to the account owner's own inbox; enabling codes before moving to a verified sending domain would lock every other user out. Turning it on is a configuration change once that domain is verified.
