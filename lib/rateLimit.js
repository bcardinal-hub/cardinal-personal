...
export function rateLimit({ windowMs, max, label }) {
  return (req, res, next) => {
    // Namespaced per limiter: each limit gets its own count per IP. Keyed on
    // IP alone, every limiter shared one bucket — a few logins would quietly
    // use up that same connection's password-reset allowance.
