...
// ...</cite> annotation markup directly in the visible text instead of
// keeping it as separate structured metadata — left in, it shows up
// verbatim in the UI as literal tag soup. Strip it before we ever look for
// JSON; safe to do unconditionally since it can only ever be inside a
// quoted string value, never part of the JSON's own structural characters.
function stripCitationTags(text) {
  return text.replace(/<\/?cite[^>]*>/g, "");
}

export function extractFirstJsonArray(rawText) {
