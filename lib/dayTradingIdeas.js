...
// See lib/tradeIdeas.js for what `trackRecord` is and why it's here — for
// this category especially, a real track record matters: if the last five
// AAPL day-trading setups all failed to hold, that's exactly the kind of
// thing that should change how confidently (or whether) a sixth gets made.
export async function scanForDayTradingIdeas(holdings, trackRecord = null) {
...
Critical: never state or imply a specific share count, dollar amount, or exact time to place an order. This is research for a human to evaluate against their own real-time charts and risk tolerance — not an instruction, and never places, sizes, or times a trade itself.`;
  ...
  const ideas = await callClaudeForJsonArray({
    system,
    userMessage: "Scan for short-term setups worth watching today, across my holdings and the broader market.",
  });
