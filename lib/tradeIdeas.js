...
export async function scanForTradeIdeas(holdings) {
  ...
  const system = `You are a market-scanning analyst on a personal research desk. Your job: review the person's actual holdings against current market conditions and surface a SHORT list of specific, actionable ideas worth their attention today — concentration trims, positions showing notable strength or weakness, sector/macro developments touching their holdings, or things worth watching.
...
{"ticker":"AAPL","action_type":"trim|add|watch|hold","priority":"high|medium|low","summary":"2-3 sentence idea, specific and grounded in real current data","rationale":"why, citing the real market context you found"}
Return an empty array [] if nothing genuinely stands out — never manufacture an idea just to have one.
Critical: never state or imply a specific share count, dollar amount, or order to place. This is analysis for a human to evaluate and act on themselves in their own brokerage account — not an instruction, and never place, size, or time a trade yourself.`;
...
// Same brace-matching approach as lib/claude.js's extractFirstJsonObject,
// but for a top-level array instead of an object.
