...
export async function scanForTradeIdeas(holdings, trackRecord = null) {
...
${trackRecord ? `Your own real track record on past ideas in this category (real price moves since each callout, not a judgment call) — use this to calibrate, and don't repeat a pattern that hasn't worked:\n${trackRecord}\n` : ""}Use web search for real, current information — don't invent prices, news, or dates. A stock's real current price is exactly the kind of thing to verify via search before treating it as "under $40," not assume from memory.
