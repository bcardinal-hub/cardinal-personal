...
// See lib/tradeIdeas.js for what `trackRecord` is and why it's here.
// `earnings` is real scheduled earnings-date data from Finnhub's calendar
// (lib/finnhub.js getEarningsCalendar) — passed in as hard data because a
// spot check found the model's own web-searched dates off by a day, and
// an earnings date being wrong is exactly the kind of error that makes an
// options timing idea useless.
export async function scanForOptionsIdeas(holdings, trackRecord = null, earnings = []) {
...
Liquidity matters more than the strategy logic: only surface an idea for large, well-known, heavily-traded names where real options liquidity actually exists (deep, actively-traded options chains — the kind of company where you'd expect tight bid-ask spreads on its options). Skip a held ticker entirely, even if the strategy logic looks appealing on paper, if it's a smaller or thinly-traded company where real-world options liquidity would likely be poor — a theoretically sound strategy on an illiquid options chain is a bad idea in practice, not a good one.
Their current real holdings: ${holdingsSummary}.
${earnings.length ? `Real scheduled earnings dates for these holdings (hard data from a market-data provider, NOT a search result — treat these as authoritative and never contradict or "correct" them with a searched date):\n${earnings.map((e) => `${e.symbol}: ${e.date}${e.hour === "amc" ? " after close" : e.hour === "bmo" ? " before open" : ""} (${e.daysAway} days away)`).join("\n")}\n` : ""}${trackRecord ? `Your own real track record on past options ideas (real price moves since each callout, not a judgment call) — use this to calibrate:\n${trackRecord}\n` : ""}Use web search for real current context — recent price action, why a strategy might fit right now.
...
This is strategy research, not an instruction — the person evaluates suitability for their own account (options require separate broker approval) and places any trade themselves.`;
  ...
  const ideas = await callClaudeForJsonArray({
    system,
    userMessage: "What options strategies, if any, are worth me researching against my current holdings?",
  });
